...
/// and <c>{who}</c> (no format at all) becomes <c>{who:&lt;PropertyName&gt;}</c>.
/// </summary>
internal static class AddLogPropertyTagFormatMigration
{
    ...
    /// with the guessed property name it now captures, or <see langword="null"/> if <paramref name="hole"/>
    /// already captures a structured property.
    /// </summary>
    public static (InterpolationSyntax OldHole, InterpolationSyntax NewHole, string PropertyName)? TryCreateReplacement(
        InterpolationSyntax hole, SemanticModel semanticModel)
    {
        ...
        var remainingFormat = GetDisplayFormat(existingFormat);
        var propertyName = GuessPropertyName(hole.Expression, semanticModel);

        var newFormatText = remainingFormat is null ? $"<{propertyName}>" : $"<{propertyName}>{remainingFormat}";
    ...
    /// <c>&lt;...</c> with no closing <c>&gt;</c>, or the <c>&lt;&gt;</c> opt-out) is stripped from
    /// <paramref name="format"/>, or <see langword="null"/> if there's no format at all, or nothing
    /// is left over. Mirrors the runtime's own <c>TagFormat</c> parsing (RandomSkunk.StructuredLogging's
    /// Internal/LogPropertyTagFormat.cs).
    /// </summary>
    private static string? GetDisplayFormat(string? format)
    {
        if (string.IsNullOrEmpty(format))
            return null;

        return format![0] != '<' ? format : LogPropertyTagFormatParsing.GetRemainingFormat(format);
    }
