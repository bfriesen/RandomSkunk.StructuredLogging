...
/// <see cref="IOperationLog"/> - the type actually returned by <c>BeginOperation</c> - extends this interface
/// with <see cref="IOperationLog.SetException"/>/<see cref="IOperationLog.SetResult{T}"/>, which set the
/// structured properties of the operation's one eventual log entry. A sub-operation, returned by
/// <see cref="BeginSubOperation(string)"/>, is only ever given out as an <see cref="ISubOperationLog"/> -
/// deliberately missing those two members, so it's a compile error to try to set the root's exception or
/// result from a sub-operation reference. Every sub-operation shares the root operation's journal, so once
/// the root <see cref="IOperationLog"/> has been disposed, calling any member other than
/// <see cref="Properties"/>, <see cref="EventId"/>, or <see cref="IsEnabled"/> on a sub-operation still
/// referenced from outside the root's <c>using</c> scope throws <see cref="ObjectDisposedException"/>, rather
/// than risking corruption of an unrelated operation's journal. That includes <see cref="IDisposable.Dispose"/>:
/// it's only safe to call once per sub-operation - disposing one after the root is already disposed throws
/// like everything else.
/// </summary>
public interface ISubOperationLog : IDisposable
{
    /// <summary>
    ...
    ISubOperationLog Escalate(LogLevel level);
    ...
    /// <returns>This <see cref="ISubOperationLog"/>, so calls can be chained.</returns>
    /// <exception cref="ObjectDisposedException">The root operation has already been disposed.</exception>
    ISubOperationLog AddProperty<T>(string name, T value);
    ...
    /// <returns>This <see cref="ISubOperationLog"/>, so calls can be chained.</returns>
    /// <exception cref="ObjectDisposedException">The root operation has already been disposed.</exception>
    ISubOperationLog AppendException(Exception exception);
    ...
    /// <see cref="OperationLogExtensions.AppendResultTo{T}"/> extension method rather than directly, so it
    ...
    /// <returns>This <see cref="ISubOperationLog"/>, so calls can be chained.</returns>
    /// <exception cref="ObjectDisposedException">The root operation has already been disposed.</exception>
    ISubOperationLog AppendResult<T>(T value);
    ...
    /// <returns>This <see cref="ISubOperationLog"/>, so calls can be chained.</returns>
    /// <exception cref="ObjectDisposedException">The root operation has already been disposed.</exception>
    ISubOperationLog Append(string text);
    ...
    /// <returns>This <see cref="ISubOperationLog"/>, so calls can be chained.</returns>
    /// <exception cref="ObjectDisposedException">The root operation has already been disposed.</exception>
    ISubOperationLog Append([InterpolatedStringHandlerArgument("")] ref OperationLogInterpolatedStringHandler text);
    ...
    /// <returns>This <see cref="ISubOperationLog"/>, so calls can be chained.</returns>
    /// <exception cref="ObjectDisposedException">The root operation has already been disposed.</exception>
    ISubOperationLog AppendValue<T>(T value, [CallerArgumentExpression(nameof(value))] string? valueName = null);
    ...
    /// <returns>This <see cref="ISubOperationLog"/>, so calls can be chained.</returns>
    /// <exception cref="ObjectDisposedException">The root operation has already been disposed.</exception>
    ISubOperationLog AppendJson<T>(T value, [CallerArgumentExpression(nameof(value))] string? valueName = null);
